# ROS2 Jazzy and iRobot Create3 Setup Guide

Follow this document to install ROS2 (required) and setup a ROS2 connection with the iRobot Create3 (optional). It is recommended that you follow the installation process outlined below, and refer to the documentation provided by ROS2 and iRobot for detailed instructions. Refer to this document for quick reference as needed.

If you are using the CRX10 robot, you can skip the iRobot Create3 setup guide and go straight to the [CRX10 Setup Guide](https://github.com/UofI-Arcade/UofI_AdvRobotics/blob/main/Guides/Fanuc_CRX10/1.md).

Pro-tip for some of you using AI to do your coding..... (we can tell...)... I would advise against it. It does not handle ROS2 or most of what we will do in Robotics 1 and 2 very well. 
It will break, it will be buggy, it will just not work, and you will have no idea why. Take you time, do the work.

### How to Follow This Guide 
There are three main steps in this guide:

1. Install ROS2 
2. Setup/configure the iRobot Create3 (optional)
3. Install iRobot Create3 Dependencies/Libraries (optional)
...
| Link | Description | 
| :---: | :---: | 
| [ROS2: Jazzy Documentation](https://docs.ros.org/en/jazzy/index.html) | Latest ROS2 LTS version (as of Fall 2025) | 
| [ROS2: Jazzy CLI Tutorials](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools.html) | Introduction to command line tools provided by ROS2 for _debugging!_ | 
| [ROS2: Jazzy Client Lib Tutorials](https://docs.ros.org/en/jazzy/Tutorials/Beginner-Client-Libraries.html) | Introduction to ROS2 library creation and build systems. |  
| [ROS2: All Tutorials](https://docs.ros.org/en/jazzy/Tutorials.html) | Tutorials for all experience levels whereas the two above are introductory. |
...
##### Summary
This is a summary of [the ROS2 installation guide](https://docs.ros.org/en/jazzy/Installation.html) and [the colcon installation guide](https://docs.ros.org/en/jazzy/Tutorials/Beginner-Client-Libraries/Colcon-Tutorial.html).
...
3. Setup sources(ROS2 apt repository): 
    ...
    2. `export ROS_APT_SOURCE_VERSION=$(curl -s https://api.github.com/repos/ros-infrastructure/ros-apt-source/releases/latest | grep -F "tag_name" | awk -F\" '{print $4}')`
...
4. _**IMPORTANT:**_ ROS2 documentation notes that failure to update may cause catastrophic removal of vital system packages in Ubuntu 24.04.
    ...
    to install the latest version of the ROS2 development tools.
...
6. Run (Optional):
    ...
    This is optional, and it will source the ROS2 environment variables for you whe you open a new terminal (ie tell you computer that the create3 topics exist and what they are). You do not need to do this until working with a create3.
##### Install CycloneDDS
CycloneDDS operates as the ROS2 middle-ware(RMW), which manages network communication between ROS2 nodes. 
TODO: I will have to double check this and make sure its still the same MW I will have to double check this and make sure its still the same MW - DB
1. Run:
...
2. Next, instruct ROS2 to use CycloneDDS by exporting RMW_IMPLEMENTATION to to your system variables:
    ...
    * If you would like to verify that this is working, follow the 'Run the talker and listener' instructions at the bottom of [this page](http://docs.ros.org.ros.informatik.uni-freiburg.de/en/jazzy/Installation/DDS-Implementations/Working-with-Eclipse-CycloneDDS.html?highlight=cyclonedds).
...
1. Clone repository to `/opt/ros/jazzy/src` with `sudo git clone -b jazzy https://github.com/iRobotEducation/irobot_create_msgs.git`
...
1. _**Installation**_, do once. Run the following in any directory you wish, but `/home/<user-name>/Documents` aka `~` is recommended.
	...
	git clone -b jazzy https://github.com/iRobotEducation/create3_examples.git
	cd ..
